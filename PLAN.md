...
- **Platform**: Watch-only (no iOS app needed)
...
- [ ] **Step 1.1: Firebase Setup**
  - [ ] Create Firebase project at console.firebase.google.com
  - [ ] Add WatchKit Extension app to Firebase
  - [ ] Download `GoogleService-Info.plist`
  - [ ] Install Firebase SDK via Swift Package Manager
  - [ ] Configure Firebase in `heartbeatsApp.swift`
  - **Test:** App launches without crashes, Firebase initialized

- [ ] **Step 1.2: Basic Navigation Structure**
  - [ ] Create dark mode color scheme
  - [ ] Add "Setup" screen (shown if not paired)
  - [ ] Add "Main" screen (shown if paired)
  - [ ] Create playful UI components
...
- [ ] **Step 2.1: Anonymous Auth**
  - [ ] Implement Firebase Anonymous Authentication
  - [ ] Store auth ID locally in UserDefaults
  - **Test:** App gets stable user ID, survives app restart

- [ ] **Step 2.2: Generate Pairing Code**
  - [ ] Create 6-digit unique code generator (e.g., "BEAT-1234")
  - [ ] Show code on Setup screen with colorful UI
  - [ ] Save to Firestore `/users/{userId}/pairCode`
  - **Test:** Code displays, saved to Firebase, unique each time

- [ ] **Step 2.3: Enter Pairing Code**
  - [ ] Add text field to enter code with playful styling
  - [ ] Look up code in Firestore
  - **Test:** Can search for codes, handles "not found" gracefully

- [ ] **Step 2.4: Complete Pairing**
  - [ ] Save partner ID to both users' documents in Firestore
  - [ ] Store locally in UserDefaults
  - [ ] Show "Paired!" confirmation with animation
  - **Test:** Two simulators can pair, IDs saved on both sides

- [ ] **Step 2.5: Persistent State**
  - [ ] On launch, check if paired in UserDefaults
  - [ ] Skip setup if already paired
  - [ ] Show partner name/status with colorful indicator
...
- [ ] **Step 3.1: Session Request**
  - [ ] "Feel Heartbeat" button on Main screen (playful design)
  - [ ] Write session to Realtime DB `/sessions/{id}`
  - [ ] Status: "requested"
  - **Test:** Button creates DB entry, visible in Firebase console

- [ ] **Step 3.2: Session Listener**
  - [ ] Watch app listens for incoming requests
  - [ ] Show colorful notification/alert when request arrives
  - **Test:** Two devices, one requests, other receives notification
...
- [ ] **Step 3.4: Mock Heartbeat Streaming**
  - [ ] Sender sends fake BPM (60-120) every 500ms to DB
  - [ ] Receiver reads values in real-time
  - [ ] Display BPM number with pulsing animation
...
## Current Status
**Phase:** 1  
**Status:** 🟡 Ready to start Firebase setup

### Next Steps
1. Go to https://console.firebase.google.com
2. Create new project named "heartbeats"
3. Follow the setup guide in Phase 1.1

### Notes
- Architecture supports multiple partners later (Firestore schema ready)
- Using Firebase Spark plan (free tier)
- Dark mode + colorful UI throughout
- Watch connects directly to Firebase (no iOS app needed)
